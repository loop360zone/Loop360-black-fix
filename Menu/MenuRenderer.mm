...
static void RenderSettingsTab() {
    ...
    if (ImGui::Combo(Localization::Get("menu_theme"), &theme, "Dark Purple\0Ocean Blue\0Blood Red\0Matrix Green\0")) {
        ...
        Themes::Apply(theme, ImGui::GetStyle());
        Settings::Save();
    }
    ...
    ImGui::Separator();
    ImGui::TextColored({0.3f, 1.f, 0.5f, 1.f}, "Loop360 v1.0.9");
...
void Render(float screenW, float screenH) {
    ...
    ImGui::SetNextWindowPos({20.f, 70.f}, ImGuiCond_FirstUseEver);
    ImGui::SetNextWindowSize({440.f, 540.f}, ImGuiCond_FirstUseEver);
    ImGui::Begin("Loop360", nullptr, ImGuiWindowFlags_NoCollapse);

    ImGui::TextDisabled("Drag title bar to move | Tap Loop360 to close");
    ImGui::Spacing();
